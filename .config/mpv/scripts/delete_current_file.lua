...
if not name or name == "" then return false end
    ...
    if f then f:close(); return true end
        ...
        return type(path) == "string" and path:match("^%a[%a%d_+-]*://") ~= nil
        end
        ...
        local res = mp.command_native({
            name = "subprocess",
            playback_only = false,
            args = { "sh", "-c", "command -v " .. cmd .. " >/dev/null 2>&1" },
        })
        ...
        mp.commandv("show-text", msg or "", tostring(ms or 3000))
        end
        ...
        if is_protocol(path) or not file_exists(path) then
            ...
            if have_cmd("kioclient6") then
                if run_cmd({ "kioclient6", "move", path, "trash:/" }) then return true end
                    elseif have_cmd("kioclient5") then
                        if run_cmd({ "kioclient5", "move", path, "trash:/" }) then return true end
                            end

                            if have_cmd("gio") then
                                if run_cmd({ "gio", "trash", path }) then return true end
                                    end

                                    if have_cmd("trash-put") then
                                        if run_cmd({ "trash-put", path }) then return true end
                                            end

                                            show("No trash tool found (kioclient/gio/trash-put).", 5000)
                                            return false
                                            end
                                            ...
                                            if count <= 0 or pos < 0 then return end
                                                ...
                                                if count == 1 then
                                                    ...
                                                    local new_pos = (pos >= count - 1) and (pos - 1) or (pos + 1)
                                                    ...
                                                    if #key_bindings == 0 then return end
                                                        for _, name in ipairs(key_bindings) do
                                                            ...
                                                            if file_to_delete ~= "" and file_to_delete == path then
                                                                show("", 0)
                                                                if delete_file(file_to_delete) then
                                                                    ...
                                                                    if #key_bindings > 0 then return end
                                                                        local name = mp.get_script_name() .. "_confirm_delete"
                                                                        key_bindings[1] = name
                                                                        mp.add_forced_key_binding(confirm_key, name, handle_confirm_key)
                                                                        end

                                                                        local function client_message(event)
                                                                        if not event or not event.args or #event.args < 1 then return end
                                                                            local path = mp.get_property("path")
                                                                            ...
                                                                            if event.args[1] == "delete-file" and #event.args == 1 then
                                                                                if delete_file(path) then
                                                                                    ...
                                                                                    if event.args[1] == "delete-file-confirm" and #event.args >= 3 and #key_bindings == 0 then
                                                                                        confirm_key = event.args[2]
                                                                                        file_to_delete = path
                                                                                        add_bindings()
                                                                                        mp.add_timeout(10, cleanup)
                                                                                        show(event.args[3], 10000)
                                                                                        return
                                                                                        end
                                                                                        end
